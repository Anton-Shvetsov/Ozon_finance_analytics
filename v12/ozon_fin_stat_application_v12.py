...
def get_response_stocks(client_id, api_key):
    ...
    response = requests.post(url='https://api-seller.ozon.ru/v3/product/info/stocks',headers=headers,data=json.dumps(data))
    if response.status_code == 200:
...
def load_preprocess_stocks(client_id, api_key):
    ...
    df_stocks['fbs_present'] = df_stocks['stocks'].apply(lambda x: x[0]['present'])
    df_stocks['fbo_present'] = df_stocks['stocks'].apply(lambda x: x[1]['present'])
    df_stocks['fbs_reserved'] = df_stocks['stocks'].apply(lambda x: x[0]['reserved'])
    df_stocks['fbo_reserved'] = df_stocks['stocks'].apply(lambda x: x[1]['reserved'])
    df_stocks = df_stocks.drop(columns = ['stocks','product_id'])
    return df_stocks
...
def get_stat(df_finance, df_fbo, df_fbs, df_stocks, start_date, end_date):
    ...
    if not df_orders.empty:
        # df_fbo.rename(columns={'products':'items'})
        ...
        if not df_deliveries.empty:
            ...
            df_stocks = pd.merge(df_stocks,df_stocks_products_mean,on='offer_id')
