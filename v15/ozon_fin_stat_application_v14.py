...
def get_stat(df_finance, df_fbo, df_fbs, df_stocks, start_date, end_date):
    ...
    if not df_orders.empty:
        ...
        if not df_deliveries.empty:
            ...
            df_deliveries['created_at_date'] = df_deliveries.created_at.apply(lambda x: datetime.datetime.strptime(x[:19],'%Y-%m-%dT%H:%M:%S').date())
            ...
            dict_df['df_products_total'] = df_products_total
            ...
            df_deliveries['client_id'] = df_deliveries['posting_number'].apply(lambda x: x.split('-')[0]).astype(int, copy=False)
            ...
            dict_df['df_products_ordered'] = df_products_ordered

            df_status = df_orders.groupby('status').agg(
                {'status':'count','amount': ['mean', 'sum'],
                'payout': ['mean', 'sum'], 'price': ['mean', 'sum']}).round(2)
            dict_df['df_status'] = df_status
...
def process():
    ...
    # with pd.ExcelWriter(save_path, engine='xlsxwriter') as writer:
    #     df_fbs.to_excel(writer, sheet_name='amounts', startrow=0, startcol=0)
